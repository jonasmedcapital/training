class Trainings::LessonMapper
  ...
  def self.map_search lesson
    ...
    model_attributes = model_attributes.merge({ "slug" => course.slug })
    if cover.attached?
    ...
    model_attributes = model_attributes.merge({ "sharing" => course.sharing })
    ...
    model_attributes = model_attributes.merge({ "published_at?" => course.published_at? })
    model_attributes
